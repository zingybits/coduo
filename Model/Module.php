...
class Module
{
    const URL = "https://coduo.cz/core-extensions.json";
    const CACHE_KEY = 'coduo_extension_installed_list';
    const SESSION_KEY = 'coduo-core-notification-data';
    ...
    /**  @var string */
    private $filterModule = 'ZingyBits_';
    ...
    public function loadOutDatedExtensionCollection()
    {
        try {
            ...
            foreach ($extensionList as $item) {
                ...
                $item['url'] = 'https://www.coduo.com/extensions.com';
    ...
    public function getMyExtensionList()
    {
        if (empty($this->myExtensionList)) {
            foreach ($this->moduleList->getNames() as $name) {
                if (str_contains($name, $this->filterModule) && !in_array($name, $this->ignoreList)) {
